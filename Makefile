...
migrate: ## run migrations
	cli/console doctrine:migrations:migrate --no-interaction -v
...
phpstan-to-baseline: ## run phpstan with baseline generation
	sudo chmod a+rw phpstan-baseline.neon
	cli/run-at-container bin/phpstan analyse -b -c ./phpstan.neon --allow-empty-baseline
...
# executar como root para supervisor funcionar
start-crons:
	docker compose exec --user root worker bin/supervisorctl start crons:
